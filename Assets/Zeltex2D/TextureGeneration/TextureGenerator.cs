...
namespace Zeltex.Generators
{
    ...
    public class TextureGenerator : MonoBehaviour
    {
        ...
        public Color32 MySecondaryColor;
        public Color32 ThirdColor;
        ...
        private void Start()
        {
            if (IsGenerateOnStart && Application.isPlaying)
            {
                if (IsPink == -1)
                {
        ...
        {
            ClearColors(GetTexture(), ColorToClearWith);
        }
        ...
        public void Fill(Texture2D MyTexture, Color32 FillColor)
        {
            ...
            for (int i = 0; i < MyTexture.width; i++)
            {
                ...
                {
                    PixelColors[GetPixelIndex(i, j, MyTexture.width)] = FillColor;
                }
            }
            SetPixels(MyTexture, PixelColors);
        }
        public void Bricks(Texture2D MyTexture)
        {
            ...
            for (int i = 0; i < MyTexture.width; i++)
            {
                for (int j = 0; j < MyTexture.height; j++)
                {
                    PixelColors[GetPixelIndex(i, j, MyTexture.width)] = MySecondaryColor;
                }
            }
            ...
            for (int j = 0; j < MyTexture.height; j++)
            {
                for (int i = 0; i < MyTexture.width; i++)
                {
                    if (!IsAlternative)
                    {
                        ...
                        {
                            PixelColors[GetPixelIndex(i, j, MyTexture.width)] = MyColor;
                        }
                    }
                    else
                    {
                        ...
                        {
                            PixelColors[GetPixelIndex(i, j, MyTexture.width)] = MyColor;// new Color32(255, 0, 0, 255);
                        }
                    }
                    ...
                    for (int i = 0; i < MyTexture.width; i++)
                    {
                        if (j != MyTexture.height - 1) // EdgeTop
                        {
                            for (int k = 1; k <= EdgeBottom; k++)
                            {
                                ...
                                Color32 ThisColor = PixelColors[GetPixelIndex(i, j, MyTexture.width)];
                                Color32 ThisColor1 = PixelColors[GetPixelIndex(i, LinePosition, MyTexture.width)];
                                Color32 ThisColor2 = PixelColors[GetPixelIndex(Mathf.Clamp(i + 1, 0, MyTexture.width - 1), LinePosition, MyTexture.width)];
                                Color32 ThisColor3 = PixelColors[GetPixelIndex(Mathf.Clamp(i - 1, 0, MyTexture.width - 1), LinePosition, MyTexture.width)];
                                ...
                                {
                                    PixelColors[GetPixelIndex(i, j, MyTexture.width)] = new Color32(
                            ...
                            for (int k = 1; k <= EdgeLeft; k++)
                            {
                                ...
                                Color32 ThisColor1 = PixelColors[GetPixelIndex(LinePosition, j, MyTexture.width)];
                                Color32 ThisColor2 = PixelColors[GetPixelIndex(LinePosition, Mathf.Clamp(j + 1, 0, MyTexture.width - 1), MyTexture.width)];
                                Color32 ThisColor3 = PixelColors[GetPixelIndex(LinePosition, Mathf.Clamp(j - 1, 0, MyTexture.width - 1), MyTexture.width)];
                                if (ThisColor.r == MyColor.r && ThisColor.g == MyColor.g && ThisColor.b == MyColor.b
                                    && ThisColor1.r == MySecondaryColor.r && ThisColor1.g == MySecondaryColor.g && ThisColor1.b == MySecondaryColor.b
                                    && ThisColor2.r == MySecondaryColor.r && ThisColor2.g == MySecondaryColor.g && ThisColor2.b == MySecondaryColor.b
                                    && ThisColor3.r == MySecondaryColor.r && ThisColor3.g == MySecondaryColor.g && ThisColor3.b == MySecondaryColor.b
        ...
        public void NoiseMap(Texture2D MyTexture)
        {
            ...
            for (int i = 0; i < MyTexture.width; i++)
            {
                for (int j = 0; j < MyTexture.height; j++)
                {
                    ...
                    PixelColors[GetPixelIndex(i, j, MyTexture.width)] = new Color32(RedValue, GreenValue, BlueValue, 255);
                }
            }
            SetPixels(MyTexture, PixelColors);
        }
        ...
        public void Circle(Texture2D MyTexture)
        {
            ...
            for (int i = 0; i < MyTexture.width; i++)
            {
                for (int j = 0; j < MyTexture.height; j++)
                {
                    ...
                    Color32 MyColor = PixelColors[GetPixelIndex(i, j, MyTexture.width)];
        ...
        public Color32[] CreateBorderLines(Texture2D MyTexture, Color32[] PixelColors, Color32 VoroniLineColor)
        {
            for (int i = 0; i < MyTexture.width; i++)
            {
                for (int j = 0; j < MyTexture.height; j++)
                {
                    ...
                    int ThisIndex = GetIndex(MyTexture, ThisPosition);
                    ...
                    if (!AreColousEqual(ThisColor, VoroniLineColor))
                    {
                        ...
                        if (!IsPointWithinBounds(MyTexture, AbovePosition))
                            AbovePosition.y = 0;
                        if (!IsPointWithinBounds(MyTexture, BelowPosition))
                            BelowPosition.y = MyTexture.height - 1;
                        if (!IsPointWithinBounds(MyTexture, RightPosition))
                            RightPosition.x = 0;
                        if (!IsPointWithinBounds(MyTexture, LeftPosition))
                            LeftPosition.x = MyTexture.width - 1;

                        int AboveIndex = GetIndex(MyTexture, AbovePosition);
                        int BelowIndex = GetIndex(MyTexture, BelowPosition);
                        int RightIndex = GetIndex(MyTexture, RightPosition);
                        int LeftIndex = GetIndex(MyTexture, LeftPosition);
        ...
        public Color32[] CreateVoroniColors(Texture2D MyTexture, Color32[] PixelColors, int ColorVariation, int ColorCount)
        {
            ...
            {
                int PixelIndex = GetIndex(MyTexture, MyPoints[i]);
            ...
            while (OpenNodePoints.Count > 0)
            {
                ...
                {
                    int MyNodeIndex = GetIndex(MyTexture, OpenNodePoints[i]);
                    ...
                    if (IsPointWithinBounds(MyTexture, AbovePosition))
                    {
                        int AdjacentIndex = GetIndex(MyTexture, AbovePosition);
                    ...
                    if (IsPointWithinBounds(MyTexture, BelowPosition))
                    {
                        int AdjacentIndex = GetIndex(MyTexture, BelowPosition);
                        Color32 AdjacentColor = PixelColors[AdjacentIndex];
                        if (AreColousEqual(AdjacentColor, MyBackgroundColor))
                        {
                            if (IsReduceBrightness)
                    ...
                    if (IsPointWithinBounds(MyTexture, RightPosition))
                    {
                        int AdjacentIndex = GetIndex(MyTexture, RightPosition);
                        Color32 AdjacentColor = PixelColors[AdjacentIndex];
                        if (AreColousEqual(AdjacentColor, MyBackgroundColor))
                        {
                            if (IsReduceBrightness)
                    ...
                    if (IsPointWithinBounds(MyTexture, LeftPosition))
                    {
                        int AdjacentIndex = GetIndex(MyTexture, LeftPosition);
                        Color32 AdjacentColor = PixelColors[AdjacentIndex];
                        if (AreColousEqual(AdjacentColor, MyBackgroundColor))
                        {
                            if (IsReduceBrightness)
